...
{
    public static LiveDrawOverlay Instance;
    ...
    GameObject toggleBtnGo;
    Image toggleBtnImage;
    Text toggleBtnLabel;

    // 2026-08-26: "둥둥 떠있는 실시간 그리기 버튼을 도크 탭에 넣어달라" - 토글 버튼만 MrCallDockPanel의
    // "그리기" 탭 안으로 옮긴다(ModelRotateTestPanel.SetControlsEmbedParent와 동일한 역할).
    RectTransform controlsEmbedParent;

    // 뒤이은 요청: "그리기용 캔버스는 화상통화 영역이랑 같아야해" - 그리기 서페이스 자체를 독립된
    // 카메라-앞 캔버스에 두지 않고, 도크의 실제 화상통화 영상 컨테이너 안으로 옮겨서 정확히 그
    // 영역 위에만 겹치게 한다.
    RectTransform videoAreaEmbedParent;
    bool videoAreaEmbedded;
    ...
    // MrCallDockPanel "그리기" 탭에서 호출 - 토글 버튼만 그 탭 안으로 옮긴다.
    public void SetControlsEmbedParent(RectTransform parent)
    {
        if (parent == null || toggleBtnGo == null) return;
        controlsEmbedParent = parent;

        toggleBtnGo.transform.SetParent(parent, false);
        var rt = (RectTransform)toggleBtnGo.transform;
    ...
    // MrCallDockPanel Start()에서 호출 - 그리기 서페이스가 도크의 실제 화상통화 영상 영역과
    // 정확히 겹치게 한다("그리기용 캔버스는 화상통화 영역이랑 같아야해").
    // 2026-08-26: 처음엔 drawSurfaceRect를 videoContainer 밑으로 실제 재부모 지정했는데, 통화가
    // 끝나거나 새로 시작될 때 영상 컨테이너의 자식들이 정리되면서(추정) 그 안에 들어가 있던 이
    // 오브젝트까지 같이 파괴돼버렸다(실기 확인: MissingReferenceException, 캔버스도 사라짐).
    // 그래서 재부모 지정 대신, drawSurfaceRect는 계속 LiveDrawOverlay 자신의(DontDestroyOnLoad)
    // 캔버스 밑에 두고 - 매 프레임 videoContainer의 월드 위치/크기를 그대로 따라가게(MatchVideoArea)
    // 만든다. 파괴될 일이 없는 대상을 그냥 "쫓아가기"만 하는 방식이라 안전하다.
    public void SetVideoAreaEmbedParent(RectTransform parent)
    {
        videoAreaEmbedParent = parent;
        videoAreaEmbedded = parent != null;
        if (drawSurfaceRect != null)
        {
            drawSurfaceRect.anchorMin = Vector2.zero;
            drawSurfaceRect.anchorMax = Vector2.one;
            drawSurfaceRect.offsetMin = Vector2.zero;
            drawSurfaceRect.offsetMax = Vector2.zero;
        }
    }

    // videoAreaEmbedParent(도크의 영상 컨테이너)의 월드 위치/회전/크기에 canvasGo를 맞춘다 -
    // 카메라 쪽으로 살짝 띄워서 실제 영상보다 항상 앞에 겹쳐 보이게 한다.
    void MatchVideoArea()
    {
        var target = videoAreaEmbedParent;
        var cam = Camera.main;
        Vector3 towardCamera = cam != null ? -cam.transform.forward * 0.01f : Vector3.zero;

        canvasGo.transform.position = target.TransformPoint(target.rect.center) + towardCamera;
        canvasGo.transform.rotation = target.rotation;

        Vector3 worldSize = target.TransformVector(new Vector3(target.rect.width, target.rect.height, 0f));
        float scaleX = Mathf.Abs(worldSize.x) / 1000f; // canvasGo 자체 sizeDelta(1000x560) 기준
        float scaleY = Mathf.Abs(worldSize.y) / 560f;
    ...
    void SetDrawMode(bool on)
    {
        ...
        toggleBtnImage.color = on ? MrUiTheme.Danger : MrUiTheme.AccentSoft;
    }
    ...
    void FollowCamera()
    {
        if (videoAreaEmbedded)
        {
            if (videoAreaEmbedParent == null) { videoAreaEmbedded = false; return; } // 대상(도크)이 파괴됨 - 임베드 해제, 다음 프레임은 카메라 추적으로 복귀
            MatchVideoArea();
            return;
        }
        var cam = Camera.main;
        ...
        if (canvasGo.transform.parent != cam.transform)
        {
            ...
            // Z/스케일을 MrUiTheme.HudDepth/HudScale로 통일 - WorkGuidePanel/ModelRotateTestPanel과
            // 같은 "층"에 떠 있는 것처럼 보이게 한다(Y만 달라서 서로 안 겹치고 세로로 떨어져 있음).
            canvasGo.transform.localPosition = new Vector3(0f, 0.05f, MrUiTheme.HudDepth);
    ...
    void BuildUI()
    {
        ...
        // 예전엔 서페이스가 독립된 빈 공간에 떠 있어서 위치 파악용으로 옅게 깔아뒀던 배경인데,
        // 이제 서페이스가 실제 화상통화 영상 위에 그대로 겹치므로(SetVideoAreaEmbedParent) 이걸
        // 계속 켜두면 통화 내내 영상이 어둡게 보인다 - 꺼둔다.
        ...
        toggleBtnGo = new GameObject("ToggleBtn");
        toggleBtnGo.transform.SetParent(panel.transform, false);
        toggleBtnImage = toggleBtnGo.AddComponent<Image>();
        MrUiTheme.Round(toggleBtnImage, 16);
        var btn = toggleBtnGo.AddComponent<Button>();
        ...
        labelGo.transform.SetParent(toggleBtnGo.transform, false);
        ...
        toggleBtnLabel.color = MrUiTheme.Ink;
